...
def run_macos_full_workflow(device: str, global_config: GlobalConfig) -> SuiteResult:
    ...
    # Final recommendation based on SMART health
    if health_state == "HEALTHY":
        final_rec = "SAFE TO USE"
        status = Severity.OK
    elif health_state == "WARNING":
        final_rec = "USE WITH CAUTION"
        status = Severity.WARNING
    elif health_state == "FAILING":
        final_rec = "DO NOT TRUST"
        status = Severity.CRITICAL
    else:
        final_rec = "USE WITH CAUTION (SMART health unknown)"
        status = Severity.UNKNOWN

    explanation_parts = [
        f"SMART health state: {health_state}.",
    ]
    ...
    final_check = CheckResult(
        ...
        details={"final_recommendation": final_rec, "health_state": health_state, "health_score": health_score},
        recommendations=[
